...
object PermissionUtils {
            ...
            Manifest.permission.WRITE_SETTINGS -> isGrantedSettingPermission(context)
                ...
                if (!isHightAndroid12()) {
                    if (permission == Manifest.permission.BLUETOOTH_SCAN) {
                        return normalCheck(context,Manifest.permission.ACCESS_COARSE_LOCATION)
                    }
                ...
                if (!isHightAndroid10()) {
                    if (Manifest.permission.ACCESS_BACKGROUND_LOCATION == permission) {
                        return normalCheck(context,Manifest.permission.ACCESS_FINE_LOCATION)
                    }

                    if (Manifest.permission.ACTIVITY_RECOGNITION == permission){
                        return normalCheck(context,Manifest.permission.BODY_SENSORS)
                    }
                ...
                if (!isHightAndroid8()) {
                    ...
                    if (Manifest.permission.READ_PHONE_NUMBERS == permission){
                        return normalCheck(context,Manifest.permission.READ_PHONE_STATE)
                    }
                }
    ...
    fun isAllGrantedPermission(context: Context, permissions: List<String>) =
        permissions.find { !isGrantedPermission(context, it) } == null
    ...
    fun isDeniedForever(activity: Activity, permissions: List<String>) =
        permissions.any { isDeniedForever(activity, it) }
    ...
    private fun normalCheck(context: Context, permission: String) = context.checkSelfPermission(permission) == PackageManager.PERMISSION_GRANTED
